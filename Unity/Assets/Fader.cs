...
public class Fader : MonoBehaviour, IEventListener
{   
	...
	{		
		...
		eM.AddListener(this as IEventListener, "FadeInEvent");
		eM.AddListener(this as IEventListener, "FadeOutEvent");
	}
    ...
    {   
		...
		if(m_CurrentScreenOverlayColor == m_TargetScreenOverlayColor && !eventSent) {
			this.eventSent = true;
			this.eM.QueueEvent(new FadeEvent(m_CurrentScreenOverlayColor.a > 0));
...
public class FadeOutEvent : IEvent {
	...
	public FadeOutEvent(float target)
	{
...
public class FadeInEvent : IEvent {
	private float duration;
	public FadeInEvent(float target)
	{
		this.duration = target;	
	}
